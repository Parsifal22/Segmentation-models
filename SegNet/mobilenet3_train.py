...
class WaterSegmentationDataset(Dataset):
    def __init__(self, img_dir, mask_dir, transform=None):
        ...
        # Get all image files with .jpg or .png extension
    ...
    def __getitem__(self, idx):
        ...
        # Get mask path with improved logic to avoid renaming certain files
        import re
        ...
        mask_name = mask_name + 'm.png' # Ensure 'm.png' suffix
        ...
        if mask is None:
            # Try alternative mask filename without appended 'm'
            alt_mask_name = mask_name.replace('m.png', '.png')
            alt_mask_path = os.path.join(self.mask_dir, alt_mask_name)
            mask = cv2.imread(alt_mask_path, cv2.IMREAD_GRAYSCALE)
            if mask is None:
                raise FileNotFoundError(f"Mask file not found or could not be loaded: {mask_path} or {alt_mask_path}")
            else:
...
def visualize_predictions(model, val_loader, num_samples=5):
    ...
    with torch.no_grad():
        for i, (images, masks) in enumerate(val_loader):
            ...
            pred_mask = (torch.sigmoid(output).cpu().numpy().squeeze() > 0.5).astype(np.uint8)
...
def main():
    ...
    # Create datasets
    train_dataset = WaterSegmentationDataset(train_img_dir, train_mask_dir)
    val_dataset = WaterSegmentationDataset(val_img_dir, val_mask_dir)
    
    # Create dataloaders
    batch_size = 4  # Increased from 1 to utilize GPU better
    ...
    # Initialize model with MobileNetV3 backbone
    model = SegNetWithMobileNetV3(num_classes=1, mode='large', pretrained=True).to(device)
    ...
    visualize_predictions(model, DataLoader(val_dataset, batch_size=1, shuffle=False))
    
    # Save the model
    torch.save(model.state_dict(), 'water_segmentation_mobilenetv3.pth')
